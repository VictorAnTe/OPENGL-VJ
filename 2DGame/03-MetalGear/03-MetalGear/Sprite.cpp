...
void Sprite::update(int deltaTime)
{
	if (currentAnimation >= 0)
	{
		...
		while (timeAnimation > animations[currentAnimation].millitsecPerKeyframe)
		{
			timeAnimation -= animations[currentAnimation].millitsecPerKeyframe;
			currentKeyframe = (currentKeyframe + 1) % animations[currentAnimation].keyframeDispl.size();
		}
		texCoordDispl = animations[currentAnimation].keyframeDispk[currentKeyframe];
	}
}
